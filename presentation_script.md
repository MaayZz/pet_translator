...
**Anas:** Because we wanted the classification to run locally in the browser, I exported the full trained Keras model — backbone and head fused together — as a single TensorFlow.js graph model per animal using `tensorflowjs_converter`. No `.tflite` conversion, no INT8 quantization. It runs entirely in the browser, with no server call needed for the classification step.
